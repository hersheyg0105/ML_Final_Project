...
def load_model(model, path="./checkpoints/MobileNet.pt", print_msg=True):
    try:
...
def channel_fraction_pruning(model, fraction=0.2):
    mask_dict={}
    for name, param in model.state_dict().items():
        if (("weight" in name) and ("feature_extractor" in name)): #choose correct names only convolutional layers
            score_list = torch.sum(torch.abs(param),dim=(1,2,3)).to('cpu') #batch size, channel, height and width, (b, c, h, w), lec 9 slide 13
            removed_idx = []
...
# 1. model dependancies
# 2. understand all names in the model architecture, are we what we need to prune
# 3. make sure for loop at bottom is working as you want it
# think in English at higher level
...
for prune_thres in np.arange(0, 1, 0.05):
    if pruning_method=='mag_prune':
        ...
        channel_fraction_pruning(model, prune_thres) #happens at one time, not iterative
        model._apply_mask()
        model = mb_pt.remove_channel(model)
        model = model.to(torch.device('cuda'))
...
plt.figure()
plt.plot(np.arange(0, 0.55, 0.05), res1[:,1])
